...
class World {
 ...
 private:
  ...
  std::unordered_map<glm::ivec2, std::unique_ptr<ChunkGenerateStructuresInfo>>
      m_chunkGenerateStructuresInfoMap;
  ...
  std::function<bool(const glm::ivec2 &, const glm::ivec2 &)>
      cmpVec2 = [this](auto &&PH1,
                       auto &&PH2) {
    return cmpVec2_impl(std::forward<decltype(PH1)>(PH1),
                        std::forward<decltype(PH2)>(PH2));
  };
  std::function<bool(const glm::ivec2 &, const glm::ivec2 &)>
      rcmpVec2 = [this](auto &&PH1,
                        auto &&PH2) {
